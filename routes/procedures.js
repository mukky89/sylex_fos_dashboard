    ...
    if (tag === 'table') {
      ...
      if (trs.length) {
        const rows = trs.map(tr => new TableRow({
          children: tr.querySelectorAll('th, td').map(c => {
            const th = (c.tagName || '').toLowerCase() === 'th';
            const runs = inlineRuns(c, th ? { bold: true } : {});
            return new TableCell({
              shading: th ? { fill: 'F1F5F9' } : undefined,
              margins: { top: 50, bottom: 50, left: 100, right: 100 },
              children: [new Paragraph({ children: runs.length ? runs : [new TextRun('')] })]
            });
          })
        }));
        paras.push(new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, borders: gridBorders('CCCCCC'), rows }));
      }
      return;
    }
...
function buildDoc(p) {
  ...
  if (p.scope && p.scope.trim() && !off.has('purpose')) {
  ...
  if (hasValidity && !off.has('validity')) {
