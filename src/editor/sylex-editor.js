...
export function createEditor(mountEl, opts = {}) {
  ...
  const defs = [
    ...
    { ic: ICONS.table, t: 'Vložiť tabuľku 3×3 (s hlavičkou)', fn: () => chain().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run() },
    { ic: '＋|', t: 'Pridať stĺpec vpravo',  fn: () => chain().addColumnAfter().run() },
    { ic: '－|', t: 'Odobrať stĺpec',         fn: () => chain().deleteColumn().run() },
    { ic: '＋═', t: 'Pridať riadok pod',      fn: () => chain().addRowAfter().run() },
    { ic: '－═', t: 'Odobrať riadok',         fn: () => chain().deleteRow().run() },
    { ic: 'H↹', t: 'Prepnúť hlavičkový riadok', fn: () => chain().toggleHeaderRow().run() },
    { ic: '⌫T', t: 'Zmazať tabuľku',          fn: () => chain().deleteTable().run() },
    { custom: 'xref' },
    { sep: true },
    { custom: 'font' },
    { custom: 'color' },
